...
//      final parts = message.split(RegExp(r'\s*(SAR|ريال|ر\.س)\s*', caseSensitive: false));    const textStyle = TextStyle(
//       color: Colors.white,
//       fontSize: 15,
//       fontWeight: FontWeight.w800,
//       height: 1.35,
...
class ChildHomePageScreen extends StatefulWidget {
  ...
  final GlobalKey? requestKey;
  final GlobalKey? prizesKey;
...
class _ChildHomePageScreenState extends State<ChildHomePageScreen> {
  ...
  final GlobalKey _keysKey = GlobalKey();
  final GlobalKey _insightsKey = GlobalKey();
  final GlobalKey _choresBtnKey = GlobalKey();
  final GlobalKey _goalsBtnKey = GlobalKey();
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (mounted && _loading) _fetchChildInfo();
  }
  ...
  Future<void> _checkTour() async {
    final prefs = await SharedPreferences.getInstance();
    final bool hasSeenTour = prefs.getBool('has_seen_child_tour_${widget.childId}') ?? false;
    if (!hasSeenTour) {
      _showAppTour();
    }
  }
  ...
  void _showAppTour() {
    ...
    void smartScroll(String identify) {
      ...
      if (identify == "header") scrollTo(_spendKey);
      else if (identify == "spend") scrollTo(_saveKey);
      else if (identify == "save") scrollTo(_keysKey);
      else if (identify == "keys" && insights.isNotEmpty) scrollTo(_insightsKey);
      else if (identify == "keys" && insights.isEmpty) scrollTo(_choresBtnKey);
      else if (identify == "insights") scrollTo(_choresBtnKey);
      else if (identify == "chores") scrollTo(_goalsBtnKey);
      else if (identify == "goals") scrollTo(_chartKey);
      else if (identify == "chart" && widget.payKey != null) scrollTo(widget.payKey!);
    }

    TargetContent buildContent(String identify, String title, String body, {bool isTop = false}) {
        ...
        builder: (context, controller) {
          return Container(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF37C4BE))),
    ...
    // 🛑 أضفنا `enableTargetTab: false` لمنع الضغط العشوائي على الشاشة
    targets.add(TargetFocus(identify: "header", keyTarget: _headerKey, enableTargetTab: false, contents: [buildContent("header", l10n.tour_child_welcome_title, l10n.tour_child_welcome_body)]));
    targets.add(TargetFocus(identify: "spend", keyTarget: _spendKey, enableTargetTab: false, contents: [buildContent("spend", l10n.tour_child_spend_title, l10n.tour_child_spend_body)]));
    targets.add(TargetFocus(identify: "save", keyTarget: _saveKey, enableTargetTab: false, contents: [buildContent("save", l10n.tour_child_save_title, l10n.tour_child_save_body)]));
    targets.add(TargetFocus(identify: "keys", keyTarget: _keysKey, enableTargetTab: false, contents: [buildContent("keys", l10n.tour_child_keys_title, l10n.tour_child_keys_body)]));
    if (insights.isNotEmpty) targets.add(TargetFocus(identify: "insights", keyTarget: _insightsKey, enableTargetTab: false, contents: [buildContent("insights", l10n.tour_child_insights_title, l10n.tour_child_insights_body)]));
    targets.add(TargetFocus(identify: "chores", keyTarget: _choresBtnKey, enableTargetTab: false, contents: [buildContent("chores", l10n.tour_child_chores_title, l10n.tour_child_chores_body, isTop: true)]));
    targets.add(TargetFocus(identify: "goals", keyTarget: _goalsBtnKey, enableTargetTab: false, contents: [buildContent("goals", l10n.tour_child_goals_title, l10n.tour_child_goals_body, isTop: true)]));
    targets.add(TargetFocus(identify: "chart", keyTarget: _chartKey, enableTargetTab: false, contents: [buildContent("chart", l10n.tour_child_chart_title, l10n.tour_child_chart_body, isTop: true)]));
    
    if (widget.payKey != null) targets.add(TargetFocus(identify: "nav_pay", keyTarget: widget.payKey!, enableTargetTab: false, contents: [buildContent("nav_pay", l10n.tour_child_nav_pay_title, l10n.tour_child_nav_pay_body, isTop: true)]));
    if (widget.requestKey != null) targets.add(TargetFocus(identify: "nav_request", keyTarget: widget.requestKey!, enableTargetTab: false, contents: [buildContent("nav_request", l10n.tour_child_nav_request_title, l10n.tour_child_nav_request_body, isTop: true)]));
    if (widget.prizesKey != null) targets.add(TargetFocus(identify: "nav_prizes", keyTarget: widget.prizesKey!, enableTargetTab: false, contents: [buildContent("nav_prizes", l10n.tour_child_nav_prizes_title, l10n.tour_child_nav_prizes_body, isTop: true)]));

    // 🔗 استبدل كود الـ TutorialCoachMark بهذا الكود المباشر والمضمون:
    TutorialCoachMark(
      targets: targets,
      colorShadow: const Color(0xFF37C4BE),
      opacityShadow: 0.85,
      hideSkip: true, 
      focusAnimationDuration: const Duration(milliseconds: 500),
      
      onFinish: () {
        // الحفظ المباشر بدون دوال خارجية لتفادي أي خطأ
        SharedPreferences.getInstance().then((prefs) {
          prefs.setBool('has_seen_child_tour_${widget.childId}', true);
        });
      },
      
      onSkip: () {
        // الحفظ المباشر + إرجاع true (مطلوبة في إصدار المكتبة لتأكيد التخطي)
        SharedPreferences.getInstance().then((prefs) {
          prefs.setBool('has_seen_child_tour_${widget.childId}', true);
        });
        return true; 
  ...
  } // 👈 تأكد أن هذا القوس يغلق دالة _showAppTour الكلية
  List<String> _getMonthNames(AppLocalizations l10n) {
    return [
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          savingBalance = _toDouble(data['saving']);
          currentPoints = data['rewardKeys'] ?? 0;
          avatarUrl = fetchedAvatar.isNotEmpty ? fetchedAvatar : null;
          _loading = false;
        });
      } else {
        setState(() => _loading = false);
              ...
              : RefreshIndicator(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Container(key: _headerKey, child: _header(l10n)),
                        const SizedBox(height: 20),
                        _balancesRow(l10n),
                        const SizedBox(height: 12),
                        Container(key: _keysKey, child: _keysBadge(l10n)),
                        const SizedBox(height: 24),
                        
                        if (insights.isNotEmpty) Container(key: _insightsKey, child: _insightsSection(l10n)),
                        if (insights.isNotEmpty) const SizedBox(height: 20), 
                        
                        _actionsGrid(l10n),
                        const SizedBox(height: 24),
                        
                        Container(key: _chartKey, child: _breakdownCard(l10n)),
                        const SizedBox(height: 40),
                      ],
                    ),
                  ),
