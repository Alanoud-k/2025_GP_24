...
class _ChildRewardsScreenState extends State<ChildRewardsScreen> {
  ...
  Future<void> _redeemReward(int rewardId, String title) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        _fetchRewardsData(); // إعادة جلب البيانات لتحديث الرصيد والحالة
        _showSuccessDialog(title);
      } else {
         final errorData = jsonDecode(res.body);
         ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(errorData['error'] ?? "Failed to redeem")));
      }
    } catch (e) {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Icon(Icons.celebration, size: 60, color: Color(0xFF37C4BE)),
            const SizedBox(height: 16),
            const Text("Yay!", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
            const SizedBox(height: 8),
            Text(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(colors: [Color(0xFFF7FAFC), Color(0xFFE6F4F3)], begin: Alignment.topCenter, end: Alignment.bottomCenter),
        ),
        child: SafeArea(
          child: Padding(
            padding: const EdgeInsets.fromLTRB(22, 18, 22, 0),
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: const [
                        Text("My Prizes", style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: Color(0xFF2C3E50))),
                        SizedBox(height: 4),
                        Text("Spend your hard-earned keys!", style: TextStyle(fontSize: 14, color: Colors.black54)),
                      ],
                    ),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Padding(
                              ...
                              child: _buildChildRewardCard(
                                reward: reward,
                                userKeys: _myKeys,
                                onRedeem: () => _redeemReward(reward['rewardid'], reward['rewardname']),
                              ),
                            );
                          },
                        ),
  ...
  Widget _buildChildRewardCard({required Map<String, dynamic> reward, required int userKeys, required VoidCallback onRedeem}) {
    final bool isRedeemed = reward['rewardstatus'] == 'Redeemed';
    final int points = reward['requiredkeys'];
    final bool canAfford = userKeys >= points;
    final Color cardColor = isRedeemed ? const Color(0xFFF9FAFB) : Colors.white;
    final Color textColor = isRedeemed ? Colors.grey : const Color(0xFF2C3E50);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(reward['rewarddescription'], style: TextStyle(fontSize: 14, color: isRedeemed ? Colors.grey.shade400 : Colors.black45)),
          const SizedBox(height: 20),
          SizedBox(
            width: double.infinity,
            height: 45,
            child: isRedeemed
                ? Container(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: const [
                        Icon(Icons.check_circle, color: Color(0xFF37C4BE), size: 20),
                        SizedBox(width: 8),
                        Text("You got this!", style: TextStyle(color: Color(0xFF37C4BE), fontWeight: FontWeight.bold)),
                      ],
                    ),
                  )
                : ElevatedButton.icon(
                    ...
                    label: Text(canAfford ? "Redeem Prize" : "Not enough keys", style: TextStyle(fontWeight: FontWeight.bold, color: canAfford ? Colors.white : Colors.grey.shade600)),
                  ),
          ),
        ],
      ),
    ...
    return Center(
      child: Column(
        ...
        children: const [
          Icon(Icons.emoji_events_outlined, size: 80, color: Colors.black12),
          SizedBox(height: 16),
          Text("No prizes available yet", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black38)),
        ],
      ),
    );
  }
