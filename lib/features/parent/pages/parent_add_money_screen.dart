...
class _ParentAddMoneyScreenState extends State<ParentAddMoneyScreen> {
  ...
  void _setupDeepLinks() {
    ...
    _linkSubscription = _appLinks.uriLinkStream.listen((Uri? uri) {
      if (uri != null) {
        debugPrint("Returned via deep link: $uri");
        if (uri.scheme == 'hasala' && uri.host == 'payment-result') {
          if (mounted) {
            final l10n = AppLocalizations.of(context)!;
            _showMessageBar("تم العودة للتطبيق وجاري معالجة الدفع...", backgroundColor: _tealMsg);
            
            // إرجاع true للصفحة السابقة (المحفظة) لتحديث الرصيد
  ...
  Future<void> _openPaymentPage(String redirectUrl) async {
    ...
    // استخدام externalApplication مهم لكي يفتح متصفح الجهاز الأساسي
    // لأن المتصفح الداخلي قد يمنع توجيهات الروابط المخصصة
    final ok = await launchUrl(uri, mode: LaunchMode.externalApplication);
