    ...
    try {
        ...
        if (totalSpending > 0) {
            insights.push({
                type: "weekly",
                title: "Weekly Spending",
                message: `You spent ${totalSpending.toFixed(2)} SAR in the last 7 days.`
            });
        } else {
            insights.push({
                type: "empty",
                title: "No Spending",
                message: "You haven't spent anything in the last 7 days."
            });
        }
        ...
        if (maxCategory && totalSpending > 0) {
            ...
            insights.push({
                type: "category",
                title: "Top Category",
                message: `You spent ${percentage}% of your money on ${maxCategory} recently.`
            });
        }
        ...
        if (goal.length > 0) {
            ...
            if (target > 0) {
                const progress = Math.round((saved / target) * 100);
                if (progress === 0) {
                    insights.push({ type: "goal-start", title: "Start Saving", message: `Start saving to reach your ${goalName} goal!` });
                } else if (progress >= 80 && progress < 100) {
                    const remaining = target - saved;
                    insights.push({ type: "goal-close", title: "Almost There", message: `Only ${remaining.toFixed(0)} SAR left to reach your ${goalName}!` });
                } else if (progress > 0 && progress < 80) {
                    insights.push({ type: "goal-progress", title: "Goal Progress", message: `Great progress! You're ${progress}% closer to your ${goalName} goal.` });
                }
            }
        }
            ...
            if (previous > 0) {
                ...
                if (change > 20) {
                    insights.push({
                        type: "increase",
                        title: "Spending Increase",
                        message: `${row.transactioncategory} spending increased by ${Math.round(change)}% recently.`
                    });
                }
            }
        });
...
export async function getChildChartData(childId, month, year, period = 'month') {
    try {
        ...
        const categoriesData = await sql`
            ...
            AND "transactiontype"::text = 'Payment'
            AND (
                (${period} = 'day' AND DATE("transactiondate") = make_date(${year}, ${month}, ${day})) OR
                (${period} = 'week' AND "transactiondate" >= make_date(${year}, ${month}, ${day}) - INTERVAL '6 days' AND "transactiondate" < make_date(${year}, ${month}, ${day}) + INTERVAL '1 day') OR
                (${period} = 'month' AND EXTRACT(MONTH FROM "transactiondate") = ${month} AND EXTRACT(YEAR FROM "transactiondate") = ${year}) OR
                (${period} = 'year' AND EXTRACT(YEAR FROM "transactiondate") = ${year})
            )
            GROUP BY "transactioncategory"
