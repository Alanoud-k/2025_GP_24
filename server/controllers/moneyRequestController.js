...
// ✅ Fix 1: Query PARENT's device tokens, not the child's —
//    this notification is an alert to the parent about the child's request.
const tokens = await sql`
  SELECT "fcmtoken"
  FROM "DeviceToken"
...
if (tokenList.length > 0) {
  await admin.messaging().sendEachForMulticast({
    tokens: tokenList,
    notification: {
      // ✅ Fix 2: Correct copy — parent is being notified of a request,
      //    not receiving money. Also fixed `amt` → `amount` (was a ReferenceError).
      title: "New Money Request 🙋",
      body: `Your child requested SAR ${Number(amount).toFixed(2)}`,
    },
  });
}    
